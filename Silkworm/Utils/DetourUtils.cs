...
public static class DetourUtils
{
    ...
    public static FastNativeDetour Create<T>(string typeName, string methodName, T to, out T original) where T : System.Delegate
    {
        return Create(Type.GetType(typeName), methodName, to, out original);
    }

    public static FastNativeDetour Create<T>(string typeName, string innerTypeName, string methodName, T to, out T original) where T : System.Delegate
    {
        return Create(GetInnerType(Type.GetType(typeName), innerTypeName), methodName, to, out original);
    }

    public static FastNativeDetour Create<T>(Type type, string innerTypeName, string methodName, T to, out T original) where T : System.Delegate
    {
        return Create(GetInnerType(type, innerTypeName), methodName, to, out original);
    }

    public static FastNativeDetour Create<T>(Type type, string methodName, T to, out T original) where T : System.Delegate
    {
        return Create(type.GetMethod(methodName, bindingFlags), to, out original);
    }

    private static FastNativeDetour Create<T>(MethodInfo method, T to, out T original) where T : System.Delegate
    {
